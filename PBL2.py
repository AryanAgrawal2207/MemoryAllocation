...
def allocate_memory():
  ...
  if algorithm == 1:
    ...
    return
  result_text.delete('1.0', tk.END)
  result_text.insert(tk.END, f"Memory Allocation using {algorithm_name} Algorithm:\n")
  for i in range(len(process_list)):
    result_text.insert(tk.END, f"Process {i+1}: Block {allocation[i]+1}\n")
root = tk.Tk()
root.title("Memory Allocation")
mem_blocks_label = tk.Label(root, text="Memory Blocks (separated by space):")
mem_blocks_entry = tk.Entry(root)
process_label = tk.Label(root, text="Process Sizes (separated by space):")
process_entry = tk.Entry(root)
algorithm_label = tk.Label(root, text="Memory Allocation Algorithm:")
algorithm_var = tk.IntVar()
algorithm_var.set(1)
first_fit_rb = tk.Radiobutton(root, text="First Fit",variable=algorithm_var, value=1)
best_fit_rb = tk.Radiobutton(root, text="Best Fit",variable=algorithm_var, value=2)
worst_fit_rb = tk.Radiobutton(root, text="Worst Fit",variable=algorithm_var, value=3)
allocate_button = tk.Button(root, text="Allocate Memory",command=allocate_memory)
result_label = tk.Label(root, text="Allocation Results:")
result_text = tk.Text(root, height=10)
mem_blocks_label.grid(row=0, column=0, padx=10, pady=10,sticky="W")
mem_blocks_entry.grid(row=0, column=1, padx=10, pady=10)
process_label.grid(row=1, column=0, padx=10, pady=10,sticky="W")
...
allocate_button.grid(row=5, column=0, columnspan=2, padx=10,pady=10)
